...
public class LeatherlingEntity extends HostileEntity implements IAnimatable, ILivingEntity {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.leatherling.walk", true));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.leatherling.idle", true));
        return PlayState.CONTINUE;
    }
